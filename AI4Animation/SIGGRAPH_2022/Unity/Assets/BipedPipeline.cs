    ...
    public class BipedPipeline : AssetPipelineSetup {
                    ...
                    for(int i=1; i<=2; i++) {
                        ...
                        foreach(Interval seq in asset.Sequences) {
                            ...
                            while(Pipeline.IsProcessing() && (start + index/Pipeline.GetEditor().TargetFramerate < end || Mathf.Approximately(start + index/Pipeline.GetEditor().TargetFramerate, end))) {
                                ...
                                BipedControllerSetup.Export(this, X, Y, tCurrent, tNext);
        ...
        public override void Finish() {
            if(Mode == MODE.ProcessAssets) {
                ...
                // }
                Debug.Log(Pipeline.GetEditor().Assets.Count);
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[0]).Export = true;
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[0]).SetSequence(0, 100, 7035);
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[1]).Export = true;
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[1]).SetSequence(0, 100, 7035);
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[2]).Export = true;
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[2]).SetSequence(0, 100, 8094);
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[3]).Export = true;
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[3]).SetSequence(0, 100, 8094);
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[4]).Export = true;
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[4]).SetSequence(0, 100, 7740);
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[5]).Export = true;
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[5]).SetSequence(0, 100, 7740);
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[6]).Export = true;
                MotionAsset.Retrieve(Pipeline.GetEditor().Assets[6]).SetSequence(0, 100, 7740);
                // MotionAsset.Retrieve(Pipeline.GetEditor().Assets[2]).Export = true;
                // MotionAsset.Retrieve(Pipeline.GetEditor().Assets[3]).Export = true;
                // MotionAsset.Retrieve(Pipeline.GetEditor().Assets[4]).Export = true;
                // MotionAsset.Retrieve(Pipeline.GetEditor().Assets[5]).Export = true;
        ...
        private class BipedControllerSetup {
            public static void Export(BipedPipeline setup, AssetPipeline.Data X, AssetPipeline.Data Y, float tCurrent, float tNext) {
                ...
                string[] contacts = new string[]{"LeftToeSite", "RightToeSite"};
            ...
            private class Container {
                ...
                public Container(BipedPipeline setup, float timestamp) {
